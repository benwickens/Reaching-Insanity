...
/**
 * Purpose:
 * Reads and saves maps and game states. 
 * <br>
...
public class FileManager {
	...
	public void save(GameState gameState){
		...
		for (int y = 0; y < grid.length; y++) {
			...
			/* get each individual cell for that grid and add its info to 
			the file*/
			for (int x = 0; x < grid.length; x++) { 
				...
				if (t.equals(CellType.EMPTY)) {
					...
					if (p1.getX() == x && p1.getY() == y) {
						...
						/* iterate over all enemies and check if location 
						 * matches up if so then add the enemies 
						 * abbreviation to the cell's output*/
						...
						for (Character e : gameState.getEnemies()) {
							if (e.getX() == x && e.getY() == y) {
								...
								for (int i = 0; i < enemyType.length(); i++) {
									if (java.lang.Character.isUpperCase
											(enemyType.charAt(i))) {
						...
						/* If there is no enemy above then we just add the cell
						 *  abbreviation*/
						if (!hasEnemy) {
					 ...
					 * so this is an if not an else if! add the item to the 
					 * output*/
					if (c.getItem() != null) {
						outputStr += ":" + itemAbbreviations.get(c.getItem());
					}

					/* equally, if the call is fire/water/ice then the player
					 *  could be on it*/
					...
					if (p1.getX() == x && p1.getY() == y) {
						...
						// otherwise we still need to add cell to the output
					} else {
						outputStr += cellAbbreviations.get(t);
					}
					/* in the case that its not a cell that something
					 *  (enemy/item/player) can be on then we still need to
					 *   output that.*/
				} else {
					/* normally just a cell abbreviation as no extra info is
					 *  needed*/
					outputStr += cellAbbreviations.get(t);
				 ...
				 * if we aren't at the end of the row then we need to add a 
				 * comma.*/
				if((x+1) < grid.length) {
					outputStr += ",";
				}
			}
			/* at this point we have dealt with the row completely and so we 
			 * need to add a new line, but only if we aren't already at the 
			 * end of the file.*/
			if((y) < grid.length) {
				outputStr += "\n";
			}
		...
		// now we have assembled the output string, just output the file!
		try {
	 ...
	 * @param level the current level number (output file named Level
	 *  <level>.txt
	 * @throws IOException if the file cannot be created.
	 */
	private void saveFile(Player p, String outputStr, int level) throws
	IOException {
		File outputFolder = new File("src/SavedGames/" + p.getName());
		if(!outputFolder.exists()) {
			outputFolder.mkdirs();	
		}
		String outputFilePath = outputFolder.getPath() + "/Level " + level
				+ ".txt";
	...
	public void readFileToGS(File levelFile, GameState gameState) {
		...
		try {
			...
			while (s.hasNextLine()) {
				...
				} else if (y == 3){ /*fourth line is the seconds spent on the
				 level*/
					...
					for (int x = 0; x < cells.length; x++) {
						...
						if (cells[x].contains(":")) {
							...
							if (extraInfo.equals("P1")) {
								...
								if (p2 != null) {
									enemies.add(new DumbTargetingEnemy(x, y 
											- 4, Direction.RIGHT,p1,p2));
								} else {
									enemies.add(new DumbTargetingEnemy(x, y
											- 4, Direction.RIGHT,p1,null));
								}
								...
								tempTeleportX = Integer.parseInt(extraInfo
										.split("-")[0]);
								tempTeleportY = Integer.parseInt(extraInfo
										.split("-")[1]);
							} else {
						...
						/* after the above, if the cell type is null then the
						...
						if (type == null) {
							...
							if (type.equals(CellType.TELEPORTER) && 
									tempTeleportX != -1) {
