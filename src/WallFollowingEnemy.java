...
public class WallFollowingEnemy extends Character {
	 ...
	 * 
	 * This method has gone over the soft cap limit as when creating the method
	 * its not possible to break it down
	 * 
	...
	private int validWallOnMove() {
		if (directionUP(CellType.WALL)) {
			...
			} else if (directionLeft(CellType.WALL) && directionRight(CellType.EMPTY)
			...
			} else if (directionRight(CellType.WALL) && directionLeft(CellType.EMPTY)
			...
			} else if (directionDown(CellType.WALL) && directionDownRight(CellType.WALL)
			...
			if (directionLeft(CellType.WALL) && directionDown(CellType.WALL)) {
			...
			} else if (directionDown(CellType.WALL) && directionDownLeft(CellType.WALL)
				...
				if (directionUPLeft(CellType.WALL) && directionUP(CellType.EMPTY)) {
					...
					vertiDir = Direction.UP;
					return y -= 1;
				} else if (currtDir == Direction.RIGHT && directionUPLeft(CellType.WALL)
		...
		} else if (directionDownRight(CellType.WALL) && directionDown(CellType.EMPTY)
				&& directionRight(CellType.EMPTY)) {
			if (vertiDir == Direction.DOWN) {
